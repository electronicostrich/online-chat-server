...
- AC-ROOM-02 — covered by the same `POST /rooms` handler via the normalized-name unique index; the spec at `e2e/specs/AC-ROOM-01-create-room.spec.ts` asserts a second user re-registering the same name with different casing/whitespace is rejected with `CONFLICT`. A dedicated `AC-ROOM-02` spec file is deferred to a follow-up that also exercises rename (`PATCH /rooms/{id}` — not yet implemented).
